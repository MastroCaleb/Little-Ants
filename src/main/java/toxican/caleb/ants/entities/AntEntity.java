...
implements Angerable, IAnimatable {
    ...
    private static final TrackedData<Byte> ANT_FLAGS = DataTracker.registerData(AntEntity.class, TrackedDataHandlerRegistry.BYTE);
    ...
    public static DefaultAttributeContainer.Builder createAntAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MAX_HEALTH, 4.0).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.5f).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 1f);
    }
    ...
    protected void initGoals() {
        ...
        this.targetSelector.add(3, new UniversalAngerGoal<AntEntity>(this, true));
    }
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<AntEntity>(this, "controller", 3, this::predicate));
    }
    ...
    protected EntityNavigation createNavigation(World world) {
        MobNavigation antNavigation = new MobNavigation(this, world){
            ...
            @Override
            public void tick() {
                if (AntEntity.this.feedGoal.isRunning()) {
                    return;
                }
                super.tick();
            }
    ...
    extends NotAngryGoal {
        ...
        public boolean canAntStart() {
            if (AntEntity.this.ticksUntilCanFeed > 0) {
                return false;
            }
            if (AntEntity.this.hasClay()) {
                return false;
            }
            Optional<BlockPos> optional = this.getLeaves();
            if (optional.isPresent()) {
                AntEntity.this.leavesPos = optional.get();
                AntEntity.this.navigation.startMovingTo((double)AntEntity.this.leavesPos.getX() - 0.5f, (double)AntEntity.this.leavesPos.getY(), (double)AntEntity.this.leavesPos.getZ() - 0.5f, 0.5f);
                return true;
            }
            AntEntity.this.ticksUntilCanFeed = MathHelper.nextInt(AntEntity.this.random, 20, 60);
            return false;
        }
        ...
        public boolean canAntContinue() {
            ...
            if (!AntEntity.this.hasLeaves()) {
                return false;
            }
            if (AntEntity.this.world.isRaining()) {
                return false;
            }
            if (this.completedNutrition()) {
                return AntEntity.this.random.nextFloat() < 0.2f;
            }
            if (AntEntity.this.age % 20 == 0 && !AntEntity.this.isLeaves(AntEntity.this.leavesPos)) {
                AntEntity.this.leavesPos = null;
                return false;
            }
            return true;
        }
        ...
        public void start() {
            ...
            this.running = true;
            AntEntity.this.resetNutritionTicks();
        }

        @Override
        public void stop() {
            if (this.completedNutrition()) {
                AntEntity.this.setHasClay(true);
            }
            this.running = false;
            AntEntity.this.navigation.stop();
            AntEntity.this.ticksUntilCanFeed = 200;
        }
        ...
        public void tick() {
            ...
            Vec3d vec3d = Vec3d.ofBottomCenter(AntEntity.this.leavesPos).add(0.0, 0.6f, 0.0);
            if (vec3d.distanceTo(AntEntity.this.getPos()) > 1.0) {
            ...
            boolean bl = AntEntity.this.getPos().distanceTo(this.nextTarget) <= 0.1;
            boolean bl2 = true;
            ...
            if (bl) {
                boolean bl3;
                boolean bl4 = bl3 = AntEntity.this.random.nextInt(25) == 0;
                if (bl3) {
                ...
                AntEntity.this.getLookControl().lookAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
            }
            if (bl2) {
                this.moveToNextTarget();
            }
            ++this.pollinationTicks;
            if (AntEntity.this.random.nextFloat() < 0.05f && this.pollinationTicks > this.lastNutritionTick + 60) {
                this.lastNutritionTick = this.pollinationTicks;
                AntEntity.this.playSound(AntsSounds.ANT_SOUND_EVENT, 1.0f, 1.0f);
            }
        }

        private void moveToNextTarget() {
            AntEntity.this.getMoveControl().moveTo(this.nextTarget.getX(), this.nextTarget.getY(), this.nextTarget.getZ(), 0.5f);
        }

        private float getRandomOffset() {
            return (AntEntity.this.random.nextFloat() * 2.0f - 1.0f) * 0.33333334f;
        }
                ...
                while ((double)j < searchDistance) {
                    ...
                    while (k <= j) {
                        ...
                        while (l <= j) {
                            ...
                            if (blockPos.isWithinDistance(mutable, searchDistance) && predicate.test(AntEntity.this.world.getBlockState(mutable))) {
    ...
    extends MeleeAttackGoal {
        ...
        public boolean canStart() {
            return super.canStart() && AntEntity.this.hasAngerTime();
        }

        @Override
        public boolean shouldContinue() {
            return super.shouldContinue() && AntEntity.this.hasAngerTime();
        }
    }
    ...
    extends NotAngryGoal {
        ...
        public void start() {
            ...
            if (blockEntity instanceof AntNestEntity) {
                ...
                anthiveBlockEntity.tryEnterHive(AntEntity.this, AntEntity.this.hasClay());
            }
        }
    }
    ...
    extends NotAngryGoal {
        ...
        @Override
        public boolean canAntStart() {
            return AntEntity.this.ticksLeftToFindHive == 0 && !AntEntity.this.hasHive() && AntEntity.this.canEnterHive();
        }
        ...
        @Override
        public void start() {
            AntEntity.this.ticksLeftToFindHive = 200;
            List<BlockPos> list = this.getNearbyFreeHives();
            if (list.isEmpty()) {
                return;
            }
            for (BlockPos blockPos : list) {
                if (AntEntity.this.moveToHiveGoal.isPossibleHive(blockPos)) continue;
                AntEntity.this.hivePos = blockPos;
                return;
            }
            AntEntity.this.moveToHiveGoal.clearPossibleHives();
            AntEntity.this.hivePos = list.get(0);
        }
    ...
    extends NotAngryGoal {
        ...
        public MoveToHiveGoal() {
            this.ticks = AntEntity.this.world.random.nextInt(10);
        ...
        @Override
        public boolean canAntStart() {
            return AntEntity.this.hivePos != null && !AntEntity.this.hasPositionTarget() && AntEntity.this.canEnterHive() && !this.isCloseEnough(AntEntity.this.hivePos) && (AntEntity.this.world.getBlockState(AntEntity.this.hivePos).isOf(AntsBlocks.DIRT_ANT_NEST) || AntEntity.this.world.getBlockState(AntEntity.this.hivePos).isOf(AntsBlocks.SAND_ANT_NEST));
        }
        ...
        public void stop() {
            ...
            this.ticksUntilLost = 0;
            AntEntity.this.navigation.stop();
            AntEntity.this.navigation.resetRangeMultiplier();
        }

        @Override
        public void tick() {
            if (AntEntity.this.hivePos == null) {
                return;
            }
            ++this.ticks;
            ...
            if (AntEntity.this.navigation.isFollowingPath()) {
                return;
            }
            if (AntEntity.this.isWithinDistance(AntEntity.this.hivePos, 16)) {
                boolean bl = this.startMovingToFar(AntEntity.this.hivePos);
                if (!bl) {
                    this.makeChosenHivePossibleHive();
                } else if (this.path != null && AntEntity.this.navigation.getCurrentPath().equalsPath(this.path)) {
                    ++this.ticksUntilLost;
                    ...
                    this.path = AntEntity.this.navigation.getCurrentPath();
                }
                return;
            }
            if (AntEntity.this.isTooFar(AntEntity.this.hivePos)) {
                this.setLost();
                return;
            }
            AntEntity.this.startMovingTo(AntEntity.this.hivePos);
        }

        private boolean startMovingToFar(BlockPos pos) {
            AntEntity.this.navigation.setRangeMultiplier(10.0f);
            AntEntity.this.navigation.startMovingTo(pos.getX(), pos.getY(), pos.getZ(), 0.5);
            return AntEntity.this.navigation.getCurrentPath() != null && AntEntity.this.navigation.getCurrentPath().reachesTarget();
        }
        ...
        private void makeChosenHivePossibleHive() {
            if (AntEntity.this.hivePos != null) {
                this.addPossibleHive(AntEntity.this.hivePos);
            }
            this.setLost();
        }
        ...
        private boolean isCloseEnough(BlockPos pos) {
            if (AntEntity.this.isWithinDistance(pos, 2)) {
                return true;
            }
            Path path = AntEntity.this.navigation.getCurrentPath();
    ...
    extends NotAngryGoal {
        ...
        @Override
        public boolean canAntStart() {
            return AntEntity.this.leavesPos != null && !AntEntity.this.hasPositionTarget() && this.shouldMoveToLeaf() && AntEntity.this.isLeaves(AntEntity.this.leavesPos) && !AntEntity.this.isWithinDistance(AntEntity.this.leavesPos, 2);
        }
        ...
        @Override
        public void tick() {
            if (AntEntity.this.leavesPos == null) {
                return;
            }
            ++this.ticks;
            if (this.ticks > this.getTickCount(600)) {
            ...
            if (AntEntity.this.isTooFar(AntEntity.this.leavesPos)) {
                AntEntity.this.leavesPos = null;
                return;
            }
            AntEntity.this.startMovingTo(AntEntity.this.leavesPos);
        }

        private boolean shouldMoveToLeaf() {
            return AntEntity.this.ticksSinceNutrition > 2400;
        }
    }
    ...
    extends Goal {
        ...
        @Override
        public boolean canStart() {
            return AntEntity.this.navigation.isIdle() && AntEntity.this.random.nextInt(10) == 0;
        }

        @Override
        public boolean shouldContinue() {
            return AntEntity.this.navigation.isFollowingPath();
        }
        ...
        public void start() {
            ...
            if (vec3d != null) {
                AntEntity.this.navigation.startMovingAlong(AntEntity.this.navigation.findPathTo(new BlockPos(vec3d), 1), 1.0);
            }
        }
        ...
        private Vec3d getRandomLocation() {
            Vec3d vec3d2;
            if (AntEntity.this.isHiveValid() && !AntEntity.this.isWithinDistance(AntEntity.this.hivePos, 22)) {
                Vec3d vec3d = Vec3d.ofCenter(AntEntity.this.hivePos);
                vec3d2 = vec3d.subtract(AntEntity.this.getPos()).normalize();
            } else {
                vec3d2 = AntEntity.this.getRotationVec(0.0f);
            }
            int i = 8;
            Vec3d vec3d3 = AboveGroundTargeting.find(AntEntity.this, 8, 7, vec3d2.x, vec3d2.z, 1.5707964f, 3, 1);
            if (vec3d3 != null) {
                return vec3d3;
            }
            return NoPenaltySolidTargeting.find(AntEntity.this, 8, 4, -2, vec3d2.x, vec3d2.z, 1.5707963705062866);
        }
    }

    public class AntRevengeGoal
    extends RevengeGoal {
        public AntRevengeGoal(AntEntity ant) {
            super(ant, new Class[0]);
        }

        @Override
        public boolean shouldContinue() {
            return AntEntity.this.hasAngerTime() && super.shouldContinue();
        }

        @Override
        protected void setMobEntityTarget(MobEntity mob, LivingEntity target) {
            if (mob instanceof AntEntity && this.mob.canSee(target)) {
    ...
    extends ActiveTargetGoal<PlayerEntity> {
        public StingTargetGoal(AntEntity ant) {
    ...
    extends Goal {
        ...
        @Override
        public boolean canStart() {
            return this.canAntStart() && !AntEntity.this.hasAngerTime();
        }

        @Override
        public boolean shouldContinue() {
            return this.canAntContinue() && !AntEntity.this.hasAngerTime();
        }
    }
