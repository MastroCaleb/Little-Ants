...
public class BrownNestTreeDecorator extends TreeDecorator {
    public static final BrownNestTreeDecorator INSTANCE = new BrownNestTreeDecorator(0.1f);
    ...
    public BrownNestTreeDecorator(float probability){
    ...
    public void generate(TestableWorld var1, BiConsumer<BlockPos, BlockState> var2, java.util.Random var3, List<BlockPos> var4, List<BlockPos> var5) {
        ...
        if (var3.nextFloat() >= this.probability) {
            ...
            for (BlockPos logPosition : var4) {
                // Pick a value from 0 (inclusive) to 6 (exclusive) and if it's 0, continue
                if (var3.nextInt(6) == 0) {
                ...
                var1.getBlockEntity(optional.get(), AntsBlocks.NEST_BLOCK_ENTITY).ifPresent(blockEntity -> {
                    ...
                    for (int j = 0; j < k; ++j) {
                        ...
                        nbtCompound.putString("id", Registry.ENTITY_TYPE.getId(AntsEntities.BROWN_ANT).toString());
