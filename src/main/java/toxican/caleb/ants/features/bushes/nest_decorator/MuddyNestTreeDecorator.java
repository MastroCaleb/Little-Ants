...
public class MuddyNestTreeDecorator extends TreeDecorator {
    ...
    public MuddyNestTreeDecorator(float probability){
            ...
            for (BlockPos logPosition : generator.getLogPositions()) {
                ...
                generator.getWorld().getBlockEntity(optional.get(), AntsBlocks.NEST_BLOCK_ENTITY).ifPresent(blockEntity -> {
                    ...
                    for (int j = 0; j < k; ++j) {
                        ...
                        nbtCompound.putString("id", Registry.ENTITY_TYPE.getId(AntsEntities.MUDDY_ANT).toString());
