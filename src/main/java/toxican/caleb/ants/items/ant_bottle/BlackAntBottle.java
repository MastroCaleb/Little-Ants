...
public class BlackAntBottle extends Item {

    public BlackAntBottle(Settings settings) {
        super(settings);
    }

    public static EntityType<BlackAntEntity> getAntType(Item item) {
        EntityType<BlackAntEntity> type = AntsEntities.BLACK_ANT;
        if(type==AntsEntities.BLACK_ANT){
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        EntityType<BlackAntEntity> type = getAntType(context.getStack().getItem());
        if (type != null) {
            BlackAntEntity antEntity = new BlackAntEntity(AntsEntities.BLACK_ANT, context.getWorld());
